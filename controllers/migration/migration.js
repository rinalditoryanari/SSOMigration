...
const migrateUser = async (req, res, next) => {
	...
	if (old_user.jabatan == "Mahasiswa") {
		...
		let kcGroupJurusan = kcGroupJabatan.subGroups.find(
		...
		kcGroupAngkatan = kcGroupJurusan.subGroups.find(
			(angkatan) => angkatan.name == kcGroupAngkatan,
		);
		...
		kcSubGroup = kcGroupJabatan.subGroups.find(
			(subgroup) => subgroup.name == "Data Migrasi",
		);
	}
	//Buat user baru
